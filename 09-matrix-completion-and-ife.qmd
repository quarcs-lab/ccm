...
CV picked $r = 0$, collapsing IFEct to TWFE. Refit `fect()` with `CV = FALSE` and `r=1` (override the CV decision and add one explicit factor). Report the ATT and compare to the chapter's $r = 0$ baseline. Does the on-impact event-time ATT shift?
...
tibble(spec = c("IFEct r = 0 (CV-selected, chapter)",
                "IFEct r = 1 (forced)"),
...
Refit `fect()` for MC with `CV = FALSE` at two hand-chosen penalty values — $\eta = 0.05$ (looser, more shrinkage) and $\eta = 0.0001$ (tighter, near-interpolation). Report the ATTs and discuss which lands closer to the CV-selected $\eta = 0.0007$.
