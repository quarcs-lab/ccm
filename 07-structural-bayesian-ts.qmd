...
Chapter 6 ran DiD on a panel doubly de-meaned by simplex unit and time weights — a frequentist estimator that returned a placebo-permutation confidence interval. This chapter replaces that machinery wholesale. Instead of two simplex problems, we put a **spike-and-slab prior** on donor regressors; instead of a placebo permutation, we report a **posterior credible interval** for the ATT.

Fit a **Bayesian structural time-series (BSTS)** model on the pre-period. Use *other states' cigarette sales* (and optionally covariates) as predictors. Project the fitted model forward as the counterfactual. The posterior over (observed − projected) gives a credible interval for the policy effect. We will write the resulting point estimate as $\widehat{\tau}_{\text{BSTS}}$ — the posterior-mean ATT — to keep it distinct from chapter 4's simplex-weight estimator, chapter 5's ridge-augmented SC, chapter 6's synthetic DiD, and chapter 8's `scpi` prediction-interval estimator.
...
The credible interval around $\widehat{\tau}_{\text{BSTS}}$ here is a Bayesian object — its width depends on the prior. The natural follow-up question is whether a **frequentist** prediction interval, built without a prior over donor weights, would tell the same story. Chapter 8 answers that with the `scpi` framework of @cattaneo2021prediction and @cattaneo2025scpi, which constructs a finite-sample prediction interval around the *synthetic-control* counterfactual from chapter 4 by separating in-sample weight uncertainty from out-of-sample forecast error.
