...
## Why a fifth synthetic-control chapter?

Chapter 4 fit a *classical* Synthetic Control: donor weights live on the simplex and are chosen by a quadratic optimiser. Chapter 5 bias-corrected that fit with a ridge outcome model (augmented synthetic control). Chapter 6 borrowed donor information in a different way — through a Bayesian structural time-series model. Chapter 7 then put a finite-sample frequentist prediction interval around the classical fit and relaxed the simplex to lasso / ridge / OLS — but, like chapters 4, 5, and 6, retained SUTVA. All of them treat the donor states' outcomes as *unaffected* by California's policy. That assumption — the **stable unit treatment value assumption (SUTVA)** — is the price of any synthetic-control estimate.
...
Classical SCM picks donor weights $w$ on the simplex that minimise the pre-treatment fit error between California's observed outcome $Y_{1,\text{pre}}$ and the donor-blended synthetic $Y_{c,\text{pre}}\, w$ — the same predictor used as $\widehat{Y_{1,t}(0)}$ in chapters 4 and 6. Formally,
...
glue("Stage 1 ATT (Classical SCM): tau_SCM_hat = ",
     "{format(round(att_classic, 2), nsmall = 2)} packs/capita")
```

Utah, Nevada, Montana, and Connecticut together carry essentially all the weight; the remaining 34 donors are flat zero. The point estimate $\widehat{\tau}_{\text{SCM}} \approx `r format(round(att_classic, 2), nsmall = 2)`$ packs/capita is recovered from a *near-deterministic* synthetic. Two questions follow: is the four-donor sparsity a feature of the data or an artefact of the simplex constraint, and is Nevada's weight contaminated by spillovers from California? Stages 2 and 3 attack these one at a time.
...
#| label: stage2-counts
#| echo: false
n_classic_active <- sum(grab_unit_weights(sc_classic)$weight > 0.01)
```

The donor pool **broadens dramatically**. About `r n_active` of 38 donors carry posterior mean weight above 0.01, versus `r n_classic_active` under the classical simplex. Crucially, even among the top-five posterior means, only Nevada's 95% credible interval excludes zero — every other top weight is statistically consistent with no contribution. The teaching point is that classical SCM's sparsity is partly a constraint artefact: when we admit posterior uncertainty over weights, the data do not strongly insist on a four-donor synthetic. Note also which donor's interval excludes zero: only Nevada's. About `r n_active` donors carry posterior mean mass, but **only one** is statistically distinguishable from a no-contribution donor — which foreshadows the Stage 3 finding that the spillover concentrates almost entirely on Nevada.

The ATT also moves — from $\widehat{\tau}_{\text{SCM}} \approx `r format(round(att_classic, 2), nsmall = 2)`$ to $\widehat{\tau}_{\text{HS}} \approx `r format(round(att_hs, 2), nsmall = 2)`$ packs/capita — and its 95% credible interval is wider than Stage 1's bootstrap CI by design. The horseshoe propagates donor-weight uncertainty into the gap series rather than treating $w$ as fixed at the optimiser's best guess. The interval still never reaches zero.
...
The posterior mean $\hat\rho \approx `r format(round(rho_hat, 3), nsmall = 3)`$ is positive (consistent with the cross-border intuition that neighbours co-move), but at ESS = `r format(round(ess_rho, 1), nsmall = 1)` the chain has barely moved from its starting value of $0$; the magnitude of $\hat\rho$ should not be over-read at the tutorial-scale `r format(MCMC_ITER, big.mark = ",")`-iteration run we use here. The callout below explains how to recover the publication-grade $\hat\rho$. The Stage 3 ATT, $\widehat{\tau}_{\text{SAR}} \approx `r format(round(att_sar, 2), nsmall = 2)`$ packs/capita, sits between $\widehat{\tau}_{\text{SCM}}$ and $\widehat{\tau}_{\text{HS}}$: adding the SAR layer *reattributes* a portion of the gap away from California's direct response toward neighbour spillovers.

:::{.callout-warning}
## ESS($\rho$) at tutorial scale is too low for a trustworthy CrI

The chapter runs the sampler at `r format(MCMC_ITER, big.mark = ",")` iterations to keep render times manageable. At that budget, the effective sample size for $\rho$ is in the single digits (`r format(round(ess_rho, 1), nsmall = 1)` in the run above) — far below the rule-of-thumb 200 — and the 95% credible interval for $\widehat{\tau}_{\text{SAR}}$ is therefore *artificially narrow*. The point estimate is recoverable (a posterior mean has low bias even at low ESS); the interval should be read as illustrative. To match the @sakaguchi2026spatial published numbers, raise `MCMC_ITER` to `100000L` and `MCMC_BURN` to `50000L` and rerender (expect ~30–90 min wall-clock).
:::
...
```{r}
#| label: fig-trace-rho
#| fig-cap: "Traceplot of rho over the post-burn iterations. At the tutorial-scale 5,000-iteration run the chain visibly fails to mix — the visual diagnosis behind a single-digit ESS. A healthy chain would look like white-noise oscillation around a stationary mean; this one drifts in long, slow excursions."
#| fig-width: 8
#| fig-height: 3
...
#| tbl-cap: "Top-8 donor states by absolute post-1988 mean spillover. Nevada absorbs the bulk of the spillover mass — roughly an order of magnitude more than the next-largest donor."
...
spill_ratio_nv <- abs(nevada_spill) / abs(spill_runner$avg_spillover)
```
...
**Nevada is the dominant spillover-receiver by more than an order of magnitude** (Nevada's mean post-1988 spillover is $\approx `r format(round(nevada_spill, 2), nsmall = 2)`$ packs/capita, roughly `r format(round(spill_ratio_nv, 1), nsmall = 1)`× the next-largest donor `r spill_runner$state`). Every other donor's spillover is small enough to be policy-noise. The story is consistent with cross-border tobacco flows reshaping consumption on both sides of the California–Nevada line — the empirical signature of SUTVA failure for this case study.
...
#| tbl-cap: "Cross-stage comparison of ATT estimates ($\\widehat\\tau_{\\text{SCM}}$, $\\widehat\\tau_{\\text{HS}}$, $\\widehat\\tau_{\\text{SAR}}$). The active-donor count jumps between Stage 1 and Stage 2 and then stays at the Stage 2 level in Stage 3 because Stage 3 conditions on the Stage-2 posterior-mean donor weights (plug-in approximation). The Stage 3 CrI is artificially narrow because ESS($\\rho$) is in single digits at the tutorial-scale MCMC budget."
...
- **Sign and order of magnitude agree.** All three estimators place the ATT in the same neighbourhood ($\widehat{\tau}_{\text{SCM}} \approx `r format(round(att_classic, 2), nsmall = 2)`$, $\widehat{\tau}_{\text{HS}} \approx `r format(round(att_hs, 2), nsmall = 2)`$, $\widehat{\tau}_{\text{SAR}} \approx `r format(round(att_sar, 2), nsmall = 2)`$ packs/capita per year), and none of the intervals reaches zero. Whatever one believes about the simplex or SUTVA, Proposition 99 reduced California cigarette consumption.
- **Active-donor count jumps once.** Heavier-tailed priors admit more donors with non-trivial mass (`r n_classic_active` $\to$ `r n_active` between Stage 1 and Stage 2); the Stage 3 count is identical to Stage 2 by construction (the SAR step holds $w$ fixed at its Stage-2 posterior mean). The sparse `r n_classic_active`-donor synthetic of Stage 1 looks like one plausible counterfactual rather than the right one.
...
The Stage 3 credible interval in the cross-stage table is the narrowest of the three, and a careless reader could conclude that the Bayesian spatial SAR is also the *most precise*. It is not. Three caveats compound at the tutorial-scale `r format(MCMC_ITER, big.mark = ",")`-iteration budget this chapter uses:

1. **ESS($\rho$) is in single digits.** The chain has barely moved from its starting value of $0$; the credible interval is built on at most a handful of effectively-independent draws, so its width is dominated by sampling noise, not by genuine posterior concentration. Raise `MCMC_ITER` to `100000L` and `MCMC_BURN` to `50000L` to recover a trustworthy interval (≈30–90 min wall-clock).
...
The point estimate $\widehat{\tau}_{\text{SAR}} \approx `r format(round(att_sar, 2), nsmall = 2)`$ packs/capita is robust to all three issues — posterior means are low-bias even at low ESS — but **intervals demand discipline**. Treat the Stage 3 CrI as illustrative until the three points above are addressed.
...
#| label: recap-helpers
#| echo: false
nv_str <- format(round(nevada_spill, 2), nsmall = 2)
```
...
| Where does the spillover land? | Almost entirely on Nevada (mean post-1988 spillover $\approx `r nv_str`$ packs/capita on Nevada); other donors are an order of magnitude smaller |
...
This is the **last chapter of Part I** and the last chapter that works the Proposition 99 case study. Across Part I we have asked the same question — did California's 1989 tobacco tax reduce per-capita cigarette consumption? — under progressively weaker assumptions: an interrupted time series (ch. 2), a 2×2 difference-in-differences (ch. 3), classical synthetic control on the simplex (ch. 4), a Bayesian structural time-series counterfactual (ch. 6), a frequentist prediction interval around the SC fit (ch. 7), and finally a Bayesian spatial relaxation of both the simplex and SUTVA (this chapter); chapter 5 stepped aside to stress-test augmented synthetic control on a purpose-built simulated panel. The headline survives every reframe: the estimated ATT is consistently in the same negative neighbourhood and never crosses zero.

Chapter 7 closed with a *frequentist* prediction interval around the synthetic counterfactual; this chapter closed with a *Bayesian* credible interval that additionally absorbs the cross-border spillover SUTVA had been hiding. Same single-treated-unit toolkit, different inferential language, complementary pictures of the same headline.

The single-treated-unit toolkit ends here. Most real-world policy data is messier: many units adopt at different dates, treatment effects evolve over event time, and parallel-trends violations need to be diagnosed rather than assumed away. Part II (chapters 9–11) leaves Proposition 99 behind and picks up the **Callaway-Sant'Anna minimum-wage county panel** (1,745 US counties × 2003–2007, three adoption cohorts), where the methods we have used so far either fail outright (one treated unit ≠ staggered cohorts) or scale awkwardly. Chapter 9 starts there with the modern staggered-DiD toolkit, generalising the *ATT on California* estimand we have estimated seven different ways here into the *ATT(g, t)* framework that lets a researcher decompose, weight, and sensitivity-test heterogeneous treatment effects across cohorts and event times.
...
- Whatever SUTVA imposes here is a *constraint on a specific donor*, not a constraint on the headline: all three ATT estimates ($\widehat{\tau}_{\text{SCM}} \approx `r format(round(att_classic, 2), nsmall = 2)`$, $\widehat{\tau}_{\text{HS}} \approx `r format(round(att_hs, 2), nsmall = 2)`$, $\widehat{\tau}_{\text{SAR}} \approx `r format(round(att_sar, 2), nsmall = 2)`$ packs/capita) sit in the same neighbourhood and none of the intervals reaches zero — the policy effect on California survives relaxing both the simplex and SUTVA.
- Replacing the simplex with a horseshoe broadens the active donor pool from `r n_classic_active` to `r n_active` — the classical sparsity is shown to be partly a constraint artefact rather than a feature of the data — but only Nevada's posterior 95% credible interval excludes zero, foreshadowing where the Stage 3 spillover lands.
- The headline derived quantity is the **Nevada spillover**: Nevada absorbs roughly `r format(round(spill_ratio_nv, 1), nsmall = 1)`× the next-largest donor's mean post-1988 spillover, the empirical signature of cross-border cigarette flows reshaping consumption on both sides of the California-Nevada line. SUTVA fails for this case study, and the SAR layer localises the failure to one neighbour.
...
- Identifying the spatial-lag parameter $\rho$ from a single observational panel is hard. At the tutorial-scale MCMC budget, ESS($\rho$) sits in the single digits (`r format(round(ess_rho, 1), nsmall = 1)` in the run above), the traceplot visibly fails to mix, and the Stage 3 credible interval is narrowest precisely because it is least trustworthy; recovering a publication-grade $\hat\rho$ requires raising iterations to 100,000 and running multiple chains with a Gelman-Rubin $\hat R$ diagnostic.
...
- The bundled `california_smoking.rda` carries only `cigsale` and `retprice` — a narrower predictor set than chapter 4 — which is why $\widehat{\tau}_{\text{SCM}}$ here is $\approx `r format(round(att_classic, 2), nsmall = 2)`$ rather than the $-18.85$ of the original Abadie replication. The methodological pipeline is identical; the inputs differ.
...
### Exercise 1: How does Utah's weight move between Stage 1 and Stage 2?
...
### Exercise 2: Year-by-year posterior probability the effect is negative
...
### Exercise 3: Distribution of active donor count across Stage 2 posterior draws

The chapter reported a posterior-*mean* active-donor count of `r n_active` (donors whose posterior mean weight exceeds 0.01). That is a *posterior-mean* statistic. The *posterior* statistic is the number of donors active *in each draw*. Plot the histogram of active-donor count (using the same 0.01 threshold) across all post-burn MCMC iterations.
...
summary(active_per_draw)
```

The per-draw active-donor count is concentrated in a relatively wide range — the model never settles on a single sparse subset of donors the way the simplex does. Each MCMC draw constructs a different sparse synthetic; the posterior-mean *summary* lights up roughly `r n_active` states only because different draws spread their weight across overlapping subsets. This is the horseshoe behaving as intended: model uncertainty is encoded in the posterior over which donors matter, not buried in a single point estimate.
:::

### Exercise 4: How concentrated is the Stage 3 spillover?
...
list(nevada_share    = shares$share[shares$state == "Nevada"],
     ...
     top3_states     = shares$state[1:3])
```

Nevada absorbs the bulk of the total absolute spillover by itself; the top-3 share approaches 1, leaving the remaining 35 donors with essentially no policy-relevant spillover mass. SUTVA fails sharply on one neighbour, not diffusely across the donor pool. That is a much friendlier diagnosis than the alternative — a diffuse failure would make every donor a leaky control and would force a much wider re-examination of the chapter 4 / 6 estimates.
:::

### Exercise 5 (stretch): Stage 1 in-time placebo at 1980
...
The placebo "ATT" is much smaller in absolute value than the real Stage-1 estimate $\widehat{\tau}_{\text{SCM}} \approx `r format(round(att_classic, 2), nsmall = 2)`$. The classical SCM machinery is not manufacturing comparable gaps when no treatment occurred in the pseudo-post window — the real estimate is not a generic artefact of the pipeline. A placebo magnitude approaching the real ATT would have been a five-alarm signal; the small magnitude here corroborates Stage 1's headline.
:::
