...
- Adding even one outside comparison unit dramatically shrinks the estimated effect. The naive within-California drop of $-27$ packs collapses to roughly $-5.7$ packs once Nevada's own $-21.3$ pack decline is subtracted off, and the resulting ATT is statistically indistinguishable from zero.
...
- A failed pre-trends test reframes the chapter's headline. The shrinkage from $-27$ to $-5.7$ is not just "Nevada has its own decline" but "California was already separating from Nevada in the policy's direction" — meaning DiD against Nevada absorbs genuine treatment signal into a non-parallel counterfactual.
...
Extend the 2×2 panel to **three control states** — Nevada, Montana, and Colorado — keeping California treated and the 1984–1993 window. Fit a two-way-fixed-effects-style regression `lm(cigsale ~ state + prepost + treat:prepost)` where `treat = (state == "California")`, and compare the DiD interaction to the Nevada-only and Montana-only versions above.
