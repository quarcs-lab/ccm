...
did_att     <- unname(did_coefs[["stateCalifornia:prepostPost"]])
nv_change   <- unname(did_coefs[["prepostPost"]])
ca_extra    <- unname(did_coefs[["stateCalifornia"]])  # CA - NV level shift in pre
hac_vcov    <- sandwich::vcovHAC(fit_did)
did_se      <- sqrt(diag(hac_vcov))[["stateCalifornia:prepostPost"]]
did_t       <- did_att / did_se
did_p       <- 2 * pt(-abs(did_t), df = fit_did$df.residual)

# Cell means (for the 2x2 grid and the within-CA pre-to-post change).
cell_means <- prop99_did |>
  group_by(state, prepost) |>
  summarize(mean_cigsale = mean(cigsale), .groups = "drop")
ca_pre   <- cell_means$mean_cigsale[cell_means$state == "California" & cell_means$prepost == "Pre"]
ca_post  <- cell_means$mean_cigsale[cell_means$state == "California" & cell_means$prepost == "Post"]
nv_pre   <- cell_means$mean_cigsale[cell_means$state == "Nevada"     & cell_means$prepost == "Pre"]
nv_post  <- cell_means$mean_cigsale[cell_means$state == "Nevada"     & cell_means$prepost == "Post"]
ca_change_22 <- ca_post - ca_pre
```

**Reading the output.** The interaction coefficient $\widehat{\tau}_{\text{DiD}}$ (R name: `stateCalifornia:prepostPost`) is `r sprintf("%+.2f", did_att)` packs (HAC SE $\approx$ `r sprintf("%.2f", did_se)`, $p \approx$ `r sprintf("%.2f", did_p)`). That is *dramatically* smaller than California's own within-state pre-to-post change of `r sprintf("%+.1f", ca_change_22)` packs over this 1984–1993 window, and statistically indistinguishable from zero. Why? Because the `prepostPost` main effect — Nevada's own pre-to-post change — is also large at `r sprintf("%+.2f", nv_change)` packs. When DiD subtracts that Nevada change from California's change, almost all of California's drop is absorbed.
...
- Adding even one outside comparison unit dramatically shrinks the estimated effect. California's within-state pre-to-post change of `r sprintf("%+.1f", ca_change_22)` packs over the 1984–1993 window collapses to $\widehat{\tau}_{\text{DiD}} \approx$ `r sprintf("%+.2f", did_att)` packs once Nevada's own `r sprintf("%+.1f", nv_change)`-pack pre-to-post change is subtracted off, and the resulting ATT is statistically indistinguishable from zero.
- Inspect **parallel trends** both visually *and* formally. Regressing `cigsale ~ state * year` on the 1984–1988 pre-window yields a slope difference of `r sprintf("%+.2f", slope_diff)` packs/year ($p \approx$ `r sprintf("%.3f", slope_p)`): California was already declining faster than Nevada *before* the policy, so parallel trends is rejected by the pre-period data themselves.
...
- @cardkrueger1994minimum — *applied example* — the textbook 2×2 DiD on New Jersey vs Pennsylvania minimum wage and fast-food employment; the historical template for the design in this chapter.
- @bertrand2004how — *seminal warning* — DiD standard errors are biased toward zero under residual autocorrelation, and the cluster-robust fix requires the number of clusters to grow; the reason this chapter's inference choice is non-trivial at $G = 2$.
- @bernal2017interrupted — *tutorial* — practitioner reference for parallel-trends *visual* diagnostics in a public-health setting.
- @roth2023whats — *review* — a synthesis of the recent DiD econometrics literature, useful for orienting between the 2×2 case here and the staggered-adoption machinery of chapter 8.
- @callaway2022handbook — *handbook chapter* — a longer-form treatment of DiD for policy evaluation that motivates the move from the 2×2 design to group-time ATTs.
...
### Exercise 1: Recover the 2×2 grid by hand
...
### Exercise 2: Swap Nevada for Montana
...
### Exercise 3: Long-window parallel-trends test

The chapter's pre-trend test uses only 1984–1988 (5 years). Re-run the slope-difference test on a wider 1980–1988 window (9 years) and compare the $p$-value. Does a longer pre-window strengthen or weaken the parallel-trends rejection?
...
long_pre <- prop99 |>
  filter(state %in% c("California", "Nevada"),
         year >= 1980, year <= 1988) |>
  mutate(state = factor(state, levels = c("Nevada", "California")))

fit_pre_long <- lm(cigsale ~ state * year, data = long_pre)

ms_pretty(list("Pre-trend test (1980-1988)" = fit_pre_long),
          vcov     = sandwich::vcovHAC,
          coef_map = c("(Intercept)"          = "Nevada baseline",
                       "stateCalifornia"      = "California level shift",
                       "year"                 = "Nevada slope",
...
With nine years of pre-period data the slope-difference estimate stays negative and the standard error shrinks; the parallel-trends rejection therefore *strengthens*. The chapter's choice of 1984–1988 was not hiding a passable pre-trend — California and Nevada were already diverging at least as early as 1980. A reader who only saw the 1984–1988 test might wonder whether the rejection was a short-window artifact; this longer window rules that out.
:::

### Exercise 4: HAC vs classical OLS standard errors
...
### Exercise 5 (stretch): Pool three control states into a TWFE-like DiD
