...
The book is **in progress**: eleven method chapters are drafted with live R, organised in two parts. **Part I (chs. 1–8)** runs the canonical Proposition 99 single-treated-unit case study, except chapter 5 (augmented synthetic control), which uses a purpose-built simulated panel (`data/tobacco_sim.dta`, generated by `R/simulate_tobacco_panel.R`); **Part II (chs. 9–11)** switches to the Callaway-Sant'Anna minimum-wage county panel for staggered-adoption methods. No `eval: false` stubs remain. The preface (`index.qmd`) and the chapter-1 roadmap/decision tree are being updated to include the newly inserted chapter 5 (work in progress). Chapter 5 now renders locally; its `augsynth` dependency is GitHub-only (`ebenmichael/augsynth`). A planned cross-method comparison chapter is still to do. See `README.md` for the current chapter table.
